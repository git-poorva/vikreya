...
function analyzeFiles() {
    ...
    if (!hasMatchingFile && uploadedFiles.length > 0) {
        ...
        const friendlyService = { reimbursement: 'Reimbursement Detection', ppc: 'PPC Growth Report', reports: 'Business Reports', listing: 'Listing Health Check' }[selectedService];
        const friendlyExpected = { reimbursement: 'FBA Inventory Ledger, Returns, or Settlement report', ppc: 'Search Term Report (Advertising → Campaign Manager → download Search Term Report)', reports: 'Business Report (Reports → Business Reports → Detail Page Sales and Traffic)', listing: 'Active Listings Report (Inventory → Inventory Reports → Active Listings)' }[selectedService];
