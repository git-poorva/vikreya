    ...
    if (unknownFeeCount > 0) {
        html += `<div class="result-card" style="border-left:4px solid #8b5cf6;">
            ...
            <div class="card-body">
                ...
                ${issueSkus.filter(s => s.issues.some(i => i.type === 'UNKNOWN_FEE')).map(s => {
        ...
        <div class="card-body">
            <div style="display:flex;flex-direction:column;gap:12px;">
                ${weightIssues.length > 0 ? `<div style="background:#1c1917;padding:16px;border-radius:10px;border-left:3px solid #dc2626;">
                    ...
                    <p style="font-size:13px;color:#ccc;margin:8px 0 0 0;">SKUs: ${weightIssues.map(s => s.sku).join(', ')}. Go to Seller Central → Help → Contact Us → FBA → request remeasurement. Estimated recovery: ₹${weightIssues.reduce((sum, s) => sum + s.issues.filter(i => i.type === 'WEIGHT_OVERCHARGE').reduce((a, i) => a + i.savings, 0), 0).toLocaleString('en-IN')}</p>
                </div>` : ''}
                ${refIssues.length > 0 ? `<div style="background:#1c1917;padding:16px;border-radius:10px;border-left:3px solid #f59e0b;">
                    ...
                    <p style="font-size:13px;color:#ccc;margin:8px 0 0 0;">SKUs: ${refIssues.map(s => s.sku).join(', ')}. Verify your products are listed in the correct browse node. Wrong category = higher referral fees.</p>
                </div>` : ''}
...
function feeAuditParseReal(data) {
    ...
    data.rows.forEach(r => {
        ...
        const amtType = (r['amount-type'] || r['amount-description'] || '').toLowerCase();
        ...
        // We aggregate different fee types
        let entry = rows.find(e => e.sku === sku && e._orderId === orderId);
        if (!entry) {
            entry = { sku, asin, _orderId: orderId, principal: 0, referralFee: 0, weightHandling: 0, closingFee: 0, shippingFee: 0, otherFee: 0 };
            rows.push(entry);
        }

        if (amtType.includes('principal') || amtType.includes('productcharges') || type === 'order') {
