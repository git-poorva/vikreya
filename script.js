...
function downloadExcel(data, serviceName) {
    try {
        ...
        // Sheet 3+: Individual case details (max 10 sheets to avoid Excel limits)
        data.cases.slice(0, 10).forEach((caseData, idx) => {
            const caseRows = [
                [`Case #${caseData.index}: ${caseData.title}`],
            ...
            caseData.sections.forEach(section => {
                if (section.heading) caseRows.push([section.heading]);
                section.paragraphs.forEach(p => caseRows.push([p]));
                section.lists.forEach((item, i) => caseRows.push([`${i + 1}. ${item}`]));
                if (section.templates.length > 0) {
                    caseRows.push(['--- Template ---']);
                    section.templates.forEach(tmpl => {
                        tmpl.split('\n').forEach(line => caseRows.push([line]));
                    });
                }
                caseRows.push([]);
            });
            ...
            wsCase['!cols'] = [{ wch: 100 }];
            XLSX.utils.book_append_sheet(wb, wsCase, `Case ${idx + 1}`);
        });
